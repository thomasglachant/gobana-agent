...
if [ "$1" = "configure" ] || [ "$1" = "abort-upgrade" ] || [ "$1" = "abort-deconfigure" ] || [ "$1" = "abort-remove" ] ; then
	...
	if deb-systemd-helper --quiet was-enabled spooter_agent.service; then
		# Enables the unit on first installation, creates new
		# symlinks on upgrades if the unit file has changed.
		deb-systemd-helper enable spooter_agent.service >/dev/null || true
		deb-systemd-invoke start spooter_agent.service >/dev/null || true
	else
		# Update the statefile to add new symlinks (if any), which need to be
		# cleaned up on purge. Also remove old symlinks.
		deb-systemd-helper update-state spooter_agent.service >/dev/null || true
	fi
	...
	if [ -d /run/systemd/system ]; then
		...
		if [ -n "$2" ]; then
			deb-systemd-invoke try-restart spooter_agent.service >/dev/null || true
		fi
	fi
fi
